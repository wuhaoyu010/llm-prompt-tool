    ...
    <div v-if="showShortcutsPanel" class="shortcuts-panel">
      ...
      <div class="shortcuts-content">
        <div class="shortcut-group">
          ...
          <div class="shortcut-item"><kbd>Esc</kbd> 退出绘制</div>
        </div>
        <div class="shortcut-group">
          <div class="shortcut-title">视图操作</div>
          <div class="shortcut-item"><kbd>滚轮</kbd> 缩放</div>
          <div class="shortcut-item"><kbd>双击</kbd> 适应屏幕</div>
        </div>
        <div class="shortcut-group">
        ...
        <div class="shortcut-group">
          ...
          <div class="shortcut-item"><kbd>V</kbd> 选择工具</div>
...
const containerStyle = computed(() => ({
  ...
  cursor: currentTool.value === 'select' ? 'default' : 'crosshair'
}))
...
function drawResizeHandles(ctx, x, y, w, h) {
  ...
  const handles = [
    ...
    { x: x + w/2 - handleSize/2, y: y - handleSize/2 }, // 上中
    { x: x + w - handleSize/2, y: y - handleSize/2 }, // 右上
    { x: x + w - handleSize/2, y: y + h/2 - handleSize/2 }, // 右中
    { x: x + w - handleSize/2, y: y + h - handleSize/2 }, // 右下
    { x: x + w/2 - handleSize/2, y: y + h - handleSize/2 }, // 下中
...
function getResizeHandle(x, y, box) {
  ...
  const handleSize = 8
  const handles = [
    { name: 'nw', x: bx - handleSize, y: by - handleSize, x2: bx + handleSize, y2: by + handleSize },
    { name: 'n', x: bx + bw/2 - handleSize, y: by - handleSize, x2: bx + bw/2 + handleSize, y2: by + handleSize },
    { name: 'ne', x: bx + bw - handleSize, y: by - handleSize, x2: bx + bw + handleSize, y2: by + handleSize },
    { name: 'e', x: bx + bw - handleSize, y: by + bh/2 - handleSize, x2: bx + bw + handleSize, y2: by + bh/2 + handleSize },
    { name: 'se', x: bx + bw - handleSize, y: by + bh - handleSize, x2: bx + bw + handleSize, y2: by + bh + handleSize },
    { name: 's', x: bx + bw/2 - handleSize, y: by + bh - handleSize, x2: bx + bw/2 + handleSize, y2: by + bh + handleSize },
    { name: 'sw', x: bx - handleSize, y: by + bh - handleSize, x2: bx + handleSize, y2: by + bh + handleSize },
    { name: 'w', x: bx - handleSize, y: by + bh/2 - handleSize, x2: bx + handleSize, y2: by + bh/2 + handleSize }
  ]
  
  for (const handle of handles) {
    if (x >= handle.x && x <= handle.x2 && y >= handle.y && y <= handle.y2) {
      return handle.name
    }
  }
  return null
