...
from src.backend.database import db, TestCase, BoundingBox
from src.backend.services.image_service import create_preview_image, delete_preview_image
...
def add_test_cases_to_defect(defect_id):
    """为缺陷添加测试用例（批量上传）"""
    from flask import current_app
    from src.backend.database import Defect
