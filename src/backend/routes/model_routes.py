...
from src.backend.services.llm_service import test_model_availability, fetch_models_list, check_llm_health
...
def check_llm_health_endpoint():
    ...
    # 如果指定了模型，使用 test_model_availability 进行实际测试
    if model_name:
        result = test_model_availability(
            config.api_key if config else '',
            config.api_url if config else '',
            model_name
        )
        if result['success']:
            return jsonify({
                'status': 'online',
                'message': '服务正常',
                'details': f'模型 "{model_name}" 可用'
            })
        else:
            return jsonify({
                'status': 'offline',
                'message': result.get('error', '模型不可用'),
